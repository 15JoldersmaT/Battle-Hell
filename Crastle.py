...
pygame.display.set_caption('Battle Hell:Crastle Valley!')
...
ThradeSupport = random.randint(1,5000)
CrastleSupport = random.randint(1,5000)
#Two Sides
#Crastle Rebels
#GOD
...
class Unit:
    ...
    def takeTurn(self):
        ...
        if self.currentMissionTime >= self.maxMissionTime or self.hasTarget == False:
            ...
            for tile in mapList:
                ...
                if selectChance2 == 1 and tx  > self.x - (26 * self.attackRange) and tx < self.x + (26 *self.attackRange)  and ty > self.y - (26 * self.attackRange) and ty < self.y +(26 * self.attackRange) and self.engaged == False:
                    if ((self.faction == 'Thrade' and tile.control < 100) or (self.faction == 'Crastle' and tile.control >-107) and self.uType != 'Builder' and self.uType != 'Aircraft' and self.uType != 'Border'):
                        ...
                        if self.name == 'Thrade Regulars' and self.population < 50:
                            self.targetX = self.targetX
                            self.targetY = self.targetY
                            self.hasTarget = True
                            
                    elif self.uType == 'Builder' and ((self.faction == 'Thrade' and tile.control >= 7 and tile.control < 100) or (self.faction == 'Crastle' and tile.control <= 3 and tile.control > -107)):
                        ...
                        if chance == 1:
                            ...
                            self.targetY = ty
                            self.hasTarget = True
                    elif (self.uType == 'Border'  or self.uType =='AirCraft')and ((self.faction == 'Thrade' and tile.control >= 7 and tile.control < 100) or (self.faction == 'Crastle' and tile.control <= 3 and tile.control > -107)):
                        chance = random.randint(1,10)
                        if tile.population >= 20:
                            chance = 1
                    ...
                    elif (self.uType == 'Mountain')and ((self.faction == 'Thrade' and  tile.control < 100) or (self.faction == 'Crastle' and tile.control > -107)):
                        chance = random.randint(1,100)
        ...
        for tile in mapList:
            ...
            if tx  > self.x - 30 and tx < self.x + 30 and ty > self.y - 30 and ty < self.y +30 and (self.engaged == False or self.uType == 'Aircraft'):
                ...
                rv = 1
                if tile.river == True:
                    rv = random.randint(1,  150)
                if moveChance == 1 and rv == 1 and ((tile.control > 50  and self.faction =='Thrade') or (tile.control < 50 and self.faction =='Crastle')) :
                    

                    if self.hasTarget == False :
                        self.y = ty
                        
                        if self.tactic == 'Surge' and self.faction == 'Thrade':
                            if tx <= self.x:
                                self.x = tx
                                if self.x == 100:
                                    ...
                                    self.hasTarget = True
                                    self.tactic = 'Roam'
                        elif self.tactic == 'Surge' and self.faction == 'Crastle':
                            if tx >= self.x:
                                self.x = tx
                                if self.x == 1025:
                                    self.targetX = self.startX
    ...
    def main(self, display):
        ...
        color = (255,252,0)
        if self.name == 'Citizen Milita':
            color = (0, 150, 0)  # Standard Orange
        elif self.name == 'Crastle Regulars':
            color = (255, 255, 255)  # Coral Orange
        elif self.name == 'Red Flower Centuria':
            color = (255, 0, 0)  
        elif self.name == 'Crastle Recon Party':
            color = (235, 235, 235)  # Tomato Orange
        ...
        #print('Show')
        if self.faction == 'Thrade':
            if self.name == 'Thrade Regulars':
                color = (88, 0, 181 )  # Vivid Royal Blue
            elif self.name == 'Thrade Folk Milita':
                color = (88, 84, 80)  # Cobalt Blue
            elif self.name == 'Vanti Guard':
                color = (255, 161, 0 )  # Cobalt Blue
            elif self.name == 'Thrade Patrol':
                color = (255, 254, 0 )  # Cobalt Blue
            elif self.name == 'Thrade Combat Engineer':
        ...
        if self.faction == 'Crastle' and self.uType == 'Aircraft':
            ySpot = ySpot+ 5
            xSpot = xSpot - 5
...
class Tile:
    def __init__(self, x, y, width, height, control, population, unit, hasFort,mountain,river,Marsh):
        ...
        cityChance = random.randint(1,45)
        ...
        riverChance = random.randint(1,35)
        if riverChance == 1:
            self.river = True
            rivers = rivers+ 1
        ...
        if x > 550:
            ...
            unitChance = random.randint(1,16)
            if unitChance == 1:
                ThradeV = Unit(self.x, self.y, self.x, self.y,'Thrade Regulars', 'Milita' ,'Thrade', 1, 100, 1, 50, 50, 100, 100, 1, 'Roam', False,False, 0, 0, 0, 100,True)
                ThradeV = Unit(self.x, self.y, self.x, self.y,'Vanti Guard', 'Border' ,'Thrade', 1, 100, 1, 99, 80, 99, 400, 1, 'Roam', False,False, 0, 0, 0, 100,False)

                units.append(ThradeV)
                for i in range(2):
                    ThradeV = Unit(self.x, self.y, self.x, self.y,'Thrade Regulars', 'Milita' ,'Thrade', 1, 100, 1, 50, 50, 100, 100, 1, 'Roam', False,False, 0, 0, 0, 100,True)
            ...
            rebelChance = random.randint(1,5)
            unitChance = random.randint(1,6)
            if unitChance == 1:
                ...
                if rebelChance <= 3:
                    rebelMilita = Unit(self.x, self.y, self.x, self.y,'Citizen Milita', 'Milita' ,'Crastle', 1, 60, 1, 1, 25, 80, 20, 1, 'Roam', False,False, 0,0,0,100,False)
                    units.append(rebelMilita)
                    rebelMilita = Unit(self.x, self.y, self.x, self.y,'Citizen Milita', 'Milita' ,'Crastle', 1, 60, 1, 1, 25, 80, 20, 1, 'Roam', False,False, 0,0,0,100,False)
                    units.append(rebelMilita)

                else:    
                    rebelMilita = Unit(self.x, self.y, self.x, self.y,'Crastle Regulars', 'Border' ,'Crastle', 1, 100, 1, 50, 30, 160, 800, 1, 'Roam', False,False, 0,0,0,100,False)
                    units.append(rebelMilita)
                   
                
                #units.append(ThradeRRF)
                #units.append(ThradeOffensiveGuard)
    ...
    def main(self, display):
        ...
        if self.control < 3:
            #color = (137, 77, 186)  # Dark Teal
            color = (0, 0, 160)

        elif self.control < 7:
            #color = (197, 103, 206)  # Medium Teal
            color = (0, 0, 200)  # Medium Brown
        ...
        if self.mountain == True:
            color = (119, 49, 0)  # A dark blue color for mountains
    ...
    def takeTurn(self):
        ...
        global ThradeUnits
        global CrastleUnits
        global maxThradeUnits
        global maxCrastleUnits

        if self.control > 3 and self.control < 7:
            self.hasFort = False
        addMoneyChance = random.randint(1,60000)
        if self.control <= 3:
            addMoneyChance = random.randint(1,60000*(CrastleMoney+1))
        if self.control >= 7:
            addMoneyChance = random.randint(1,60000*(ThradeMoney+1))
        if addMoneyChance == 1:
            if self.control <= 3:
                if self.population == 20:
                    CrastleMoney = CrastleMoney + 5
                else:
                    CrastleMoney = CrastleMoney + self.population
            elif self.control >= 7:
                if self.population == 20:
                    ThradeMoney = ThradeMoney + 5
                else:
                    ThradeMoney = ThradeMoney + self.population
        buyUnitChance = random.randint(1,100)
        if buyUnitChance == 1 and self.population >= 20:
            unitChance = random.randint(1,30)
            print (maxThradeUnits)
            if self.control >= 7 and ThradeUnits < maxThradeUnits:
                if unitChance < 6 and ThradeMoney >= 1:
                    ThradeMoney = ThradeMoney - 1
                    #self,x, y,startX, startY,name,uType,faction, cost, population, tech, speed, attack, defense, attackRange,digIn, tactic, engaged,hasTarget,targetX, targetY, currentMissionTime, maxMissionTime
                    for i in range(3):
                        ThradeV = Unit(self.x, self.y, self.x, self.y,'Thrade Regulars', 'Milita' ,'Thrade', 1, 100, 1, 50, 50, 100, 100, 1, 'Roam', False,False, 0, 0, 0, 100,True)
                        units.append(ThradeV)
                ...
                elif unitChance > 26 and ThradeMoney >= 1:
                    ThradeMoney = ThradeMoney - 1

                    #f time > ThradeSupport:
                        #for i in range(1):
                            #ThradeV = Unit(self.x, self.y, self.x, self.y,'Stommist Company', 'Border' ,'Thrade', 1, 100, 1, 20, 90, 110, 500, 1, 'Roam', False,False, 0, 0, 0, 170,True)
                            #units.append(ThradeV)
                      
                    #if unitChance == 29:
                    ThradeV = Unit(self.x, self.y, self.x, self.y,'Thrade Patrol', 'Aircraft' ,'Thrade', 1, 30, 1, 140, 50, 100, 100, 1, 'Roam', False,False, 0, 0, 0, 300,False)
                    units.append(ThradeV)
            ...
            elif self.control <= 3 and CrastleUnits < maxCrastleUnits:
                if unitChance < 6 and CrastleMoney >= 1:
                    CrastleMoney = CrastleMoney - 1
                    for i in range(6):
                        rebelMilita = Unit(self.x, self.y, self.x, self.y,'Citizen Milita', 'Milita' ,'Crastle', 1, 100, 1, 1, 25, 80, 20, 1, 'Roam', False,False, 0,0,0,100,False)
                        units.append(rebelMilita)
             
                    #dandyRRF = Unit(self.x, self.y, self.x, self.y,'Royal Combat Engineer', 'Builder' ,'Crastle', 1, 100, 10, 1, 1, 1, 1, 1, 'Roam', False,False, 0, 0, 0, 500, False)
                    #units.append(dandyRRF)
              

                if unitChance > 15 and unitChance < 20 and CrastleMoney >= 1:
                    CrastleMoney = CrastleMoney - 1
                    for i in range(3):
                        rebelMilita = Unit(self.x, self.y, self.x, self.y,'Crastle Regulars', 'Border' ,'Crastle', 1, 100, 1, 50, 30, 160, 800, 1, 'Roam', False,False, 0,0,0,100,False)
                        units.append(rebelMilita)

                    #if time > CrastleSupport:
                    #    ThradeV = Unit(self.x, self.y, self.x, self.y,'Xetta Trained Regulars', 'Border' ,'Crastle', 1, 100, 1, 80, 92, 120, 700, 1, 'Roam', False,False, 0, 0, 0, 500,False)
                    #    units.append(ThradeV)
                        
                     #   XettaV = Unit(self.x, self.y, self.x, self.y,'Haax Jet', 'Aircraft' ,'Crastle', 1, 30, 1, 150, 80, 100, 500, 1, 'Roam', False,False, 0, 0, 0, 700,False)
                     #   units.append(XettaV)


                
                if unitChance > 25 and CrastleMoney >= 1:
                    CrastleMoney = CrastleMoney - 1
                    
                    for i in range(1):
                        rebelMilita = Unit(self.x, self.y, self.x, self.y,'Crastle Recon Party', 'Aircraft' ,'Crastle', 1, 30, 1, 20, 1, 60, 400, 1, 'Roam', False,False, 0,0,0,100,False)
                        units.append(rebelMilita)
...
while True:
    ...
    ThradeMLabel = myFont.render("Thrade Money:" + str(ThradeMoney), 1, (255,255,255))
    CrastleMLabel = myFont.render("Crastle Money:" + str(CrastleMoney), 1, (255,255,255))

    ThradeCLabel = myFont.render("Thrade Losses:" + str(northCas), 1, (255,255,255))
    CrastleCLabel = myFont.render("Crastle Losses:" + str(southCas), 1, (255,255,255))
    ...
    if takeTurn == True and  paused == False:
        ...
        for unit in units:
            if unit.faction == 'Crastle':
                CrastleUnits = CrastleUnits + 1
            else:
                ThradeUnits = ThradeUnits + 1
            unit.takeTurn()
        ...
        for tile in mapList:
            if tile.population == 20:
                if tile.control >= 50:
                    maxThradeUnits = maxThradeUnits + 2
                else:
                    maxCrastleUnits = maxCrastleUnits + 2
        ...
        #print ('Total Thrade Units: ' + str(ThradeUnits) + 'Thrade Max Units: ' + str(ThradeMaxUnits))
        #print ('Total Crastle Units: ' + str(CrastleUnits) +'Crastle Max Units: ' + str(CrastleMaxUnits))
        time = time+1
    ...
    ThradeULabel = myFont.render('Total Thrade Units: ' + str(ThradeUnits) + ' :: Thrade Max Units: ' + str(maxThradeUnits), 1, (255,255,255))
    CrastleULabel = myFont.render('Total Crastle Units: ' + str(CrastleUnits) +' :: Crastle Max Units: ' + str(maxCrastleUnits), 1, (255,255,255))
    ...
    #player.main(display)
    display.blit(ThradeMLabel, (20, 20))
    display.blit(CrastleMLabel, (20, 40))
    display.blit(ThradeCLabel, (20, 710))
    display.blit(CrastleCLabel, (20, 730))
    display.blit(ThradeULabel, (300, 710))
    display.blit(CrastleULabel, (300, 730))
